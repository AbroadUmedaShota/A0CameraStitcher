...
public sealed class ProgressStepViewModel : ObservableObject
{
    ...
    public ProgressStepViewModel(SimulatedTransactionState state, string label)
    {
        State = state;
        Label = label;
    }

    public SimulatedTransactionState State { get; }
    ...
    public void SetCompleted() => SetVisual("通過", Brushes.Honeydew, Brushes.DarkGreen, Brushes.SeaGreen);

    public void SetCurrent() => SetVisual("表示中", Brushes.LightGoldenrodYellow, Brushes.Black, Brushes.DarkOrange);

    public void SetFailure() => SetVisual("異常終了", Brushes.MistyRose, Brushes.DarkRed, Brushes.Firebrick);
