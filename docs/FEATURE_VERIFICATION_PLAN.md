...
| V-M3P-001 | simulated app | IPC、永続transaction、crash/restart、画面上のSimulated表示 | 実機不要/.NET 10 | Pass 2026-08-10（software-only）。Release build 0 warning/0 error、Foundation 19/19、Operator Shell 16/16、`Test-M3Simulated.ps1` Pass。実Camera AgentのD810実行は証明しない |
| V-M3P-002 | operator workflow | 起動同意、全readiness blocker、Ready/補正範囲内、active時全操作ロック、失敗別結果、mode別合成可否、明示保存、新規撮影準備 | 実機不要/.NET 10 | Pass 2026-08-10（software-only）。CAM-A-only、mode lock、no-auto-fallback、30日profile承認、fixed-local保存先、Single Stitch `NotApplicable`をOperator Shell 16/16で確認。実WPF画面操作・accessibility・実機は未検証 |
| V-MODE-001 | 明示Single/Dual mode | modeはactive前だけ変更可、required aliasesをtransactionへ固定、Dualのcamera不足でSingleへ自動降格しない | 実機不要/.NET 10 | Pass 2026-08-10（software contract）。Foundation 19/19とOperator Shell 16/16でrequired aliases固定、CAM-A-only、active中変更拒否、Single/Dual no-auto-fallbackを確認。実D810台数拒否は未検証 |
| V-M3P-SINGLE-001 | simulated Single workflow | 一回操作でCAM-Aだけを処理、original一件、他alias未開始、stitch `NotApplicable`、明示export一件、no retry | 実機不要/.NET 10 | Pass 2026-08-10（software-only）。Foundation 19/19、Operator Shell 16/16、一括script Pass。実Camera Agent経由のD810撮影、actual JPEG、canonical original実exportは証明しない |
...
| V-1CAM-005 | 実Single app workflow | WPF→Camera AgentでCAM-A撮影、canonical original review、stitch N/A、fixed-local bit-identical export、継続Live View handoff | D810一台＋専用空カード＋実Camera Agent | Unverified（hardware）。software boundaryはFoundation 19/19、Operator Shell 16/16、M3 script Pass、SDK-less／licensed-SDK-enabled Release CTest各7/7。継続Live View v2、実D810、actual JPEG、実WPF操作、10回p95、100件は未実施 |
