...
結果は`Pass`、`Partial`、`Fail`、`Deferred`で記録する。一台の証拠は`SingleCamera`の明示contractだけを証明し、`DualCamera`、二台固定リグ、実写A0品質へ読み替えない。既存のsoftware-only／Phase 0一台証拠も、2026-08-10追加の実WPF Camera Agent連携や一台製品受入の合格へ読み替えない。
...
| V-M3P-001 | simulated app | IPC、永続transaction、crash/restart、画面上のSimulated表示 | 実機不要/.NET 10 | Pass 2026-08-10（software-only）。Release build 0 warning/0 error、Foundation 20/20、Operator Shell 17/17、`Test-M3Simulated.ps1` Pass。実Camera AgentのD810実行は証明しない |
| V-M3P-002 | operator workflow | 起動同意、全readiness blocker、Ready/補正範囲内、active時全操作ロック、失敗別結果、mode別合成可否、明示保存、新規撮影準備 | 実機不要/.NET 10 | Pass 2026-08-10（software-only）。CAM-A-only、mode lock、no-auto-fallback、30日profile承認、fixed-local保存先、Single Stitch `NotApplicable`、継続Live View停止→capture→成功後再開をOperator Shell 17/17で確認。実WPF画面操作・accessibility・実機は未検証 |
| V-MODE-001 | 明示Single/Dual mode | modeはactive前だけ変更可、required aliasesをtransactionへ固定、Dualのcamera不足でSingleへ自動降格しない | 実機不要/.NET 10 | Pass 2026-08-10（software contract）。Foundation 20/20とOperator Shell 17/17でrequired aliases固定、CAM-A-only、active中変更拒否、Single/Dual no-auto-fallbackを確認。実D810台数拒否は未検証 |
| V-M3P-SINGLE-001 | simulated Single workflow | 一回操作でCAM-Aだけを処理、original一件、他alias未開始、stitch `NotApplicable`、明示export一件、no retry | 実機不要/.NET 10 | Pass 2026-08-10（software-only）。Foundation 20/20、Operator Shell 17/17、一括script Pass。実Camera Agent経由のD810撮影、actual JPEG、canonical original実exportは証明しない |
| V-1CAM-001 | CAM-A接続状態 | identity-v3がWPD digestとSDK/WPD各exactly-oneを相関し、SDK status取得後session close | D810一台、読取専用 | Partial。historical v4は2026-08-07にPass。操作者報告のexactly-oneとCAM-A identity-v3登録はPassしたが、直後のv5はlegacy alias解決defectでcamera open前停止。software修正済みで、実D810 v5のread-only再実行は未検証 |
...
| V-CARD-001 | Single one-shot回収 | exactly-one physical D810、empty-before、SDK一回撮影、WPD回収、PC原本、exact delete、empty-after | D810一台＋専用空カード | 操作者が保留中のためDeferred。既存software contractを一台製品受入へ読み替えない |
| V-1CAM-005 | 実Single app workflow | WPF→Camera AgentでCAM-A撮影、canonical original review、stitch N/A、fixed-local bit-identical export、継続Live View handoff | D810一台＋専用空カード＋実Camera Agent | Unverified（hardware）。software boundaryはFoundation 20/20、Operator Shell 17/17、M3 script Pass、SDK-less／licensed-SDK-enabled Debug/Release CTest各7/7。QA reviseでcanonical Base64、512 KiB frame、1 MiB pipe、session ownership/double-start、20秒timeout、backpressure、stop/close失敗と残留Live View時capture拒否、解析済みv2 rejection envelopeを偽SDK/clockと実Named Pipeで確認した。v2 software lifecycleは実装済みだが、実D810、actual JPEG、実WPF操作、10回handoff/p95、100件は未実施 |
...
- `DualCamera`は現行rig-profile 1.1と以下の全条件を使う。`SingleCamera`は`HG-0009`で承認するmode-specific profileを使い、identity、read-only設定、画角、crop、一台品質だけを評価し、overlap、seam、二台間相対補正を`NotApplicable`にする。未承認の間は実Single製品readinessをPassにしない。
