...
| V-M3P-001 | simulated app | IPC、永続transaction、crash/restart、画面上のSimulated表示 | 実機不要/.NET 10 | Pass 2026-08-10（software-only）。Release build 0 warning/0 error、Foundation 19/19、Operator Shell 15/15、`Test-M3Simulated.ps1` Pass。実Camera AgentのD810実行は証明しない |
| V-M3P-002 | operator workflow | 起動同意、全readiness blocker、Ready/補正範囲内、active時全操作ロック、失敗別結果、mode別合成可否、明示保存、新規撮影準備 | 実機不要/.NET 10 | Pass 2026-08-10（software-only）。明示mode、Single/Dual表示、mode lock、no-auto-fallback、Single Stitch `NotApplicable`、明示export、起動時gateをOperator Shell 15/15で確認。実WPF画面操作・accessibility・実機は未検証 |
| V-MODE-001 | 明示Single/Dual mode | modeはactive前だけ変更可、required aliasesをtransactionへ固定、Dualのcamera不足でSingleへ自動降格しない | 実機不要/.NET 10 | Pass 2026-08-10（software contract）。Foundation 19/19とOperator Shell 15/15でrequired aliases固定、active中変更拒否、Single/Dual no-auto-fallbackを確認。実D810台数拒否は未検証 |
| V-M3P-SINGLE-001 | simulated Single workflow | 一回操作で選択aliasだけを処理、original一件、他alias未開始、stitch `NotApplicable`、明示export一件、no retry | 実機不要/.NET 10 | Pass 2026-08-10（software-only）。Foundation 19/19、Operator Shell 15/15、一括script Pass。実Camera Agent経由のD810撮影、actual JPEG、canonical original実exportは証明しない |
...
| V-1CAM-005 | 実Single app workflow | WPF→Camera Agentで一台撮影、canonical original review、stitch N/A、bit-identical明示export、Live View handoff | D810一台＋専用空カード＋実Camera Agent | Unverified（hardware）。2026-08-10のsoftware boundaryはFoundation 19/19、Operator Shell 15/15、M3 script Pass、SDK-less／licensed-SDK-enabled CTest各6/6に合格したが、camera commandは送っておらず、実D810、actual JPEG、実WPF操作、`HG-0009`受入は未実施 |
