...
| V-1CAM-005 | 実Single app workflow | WPF→Camera AgentでCAM-A撮影、canonical original review、stitch N/A、fixed-local bit-identical export、継続Live View handoff | D810一台＋専用空カード＋実Camera Agent | Unverified（hardware）。software boundaryはFoundation 20/20、Operator Shell 17/17、M3 script Pass、SDK-less／licensed-SDK-enabled Release CTest各7/7。v2 software lifecycleは実装済みだが、実D810、actual JPEG、実WPF操作、10回handoff/p95、100件は未実施 |
