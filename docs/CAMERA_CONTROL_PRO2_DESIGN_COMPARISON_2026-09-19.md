...
調査日: 2026-09-19  
比較基準: 調査時`main` `c528007ff50807aacc55a71c38eb23ec8dd798dd`、PR #210統合前base `6132f12c3d619a3fa193ff9bead3e972d7d9fe66`、PR #210 head `d767dda8f761af1afacdd419d7395ecd6c3588df`
...
- `InitApplication()`はcamera接続後に`PhotoCaptured`と`CaptureCompleted`へhandlerを登録する。  
  https://github.com/dukus/digiCamControl/blob/9269e7851e5130f7d2278cc9942eccde0fd5e593/CameraControlCmd/Program.cs#L524-L545
...
- 受信済みDeviceInfoのadvertised operationsを検査し、Nikon vendor/model別に不足・不安定operationを追加・無効化するfixupを持つ。ここに列挙された機種群の分岐は、D810で同じ補正や各operationの実動作を保証する証拠ではない。
...
|---|---|---|---|
| 検出 | Nikon SDK source列挙とWPD D810列挙を別経路で実装。統合済みPR #210はWPD列挙数とstill-image compatible数を匿名記録する。 | CCP2は接続camera nameを表示。libgphoto2はdevice infoのoperationsを検査・補正する。 | software実装済み。実機WPD分類は未受入。 |
| 個体照合 | SDK/WPD identity map、alias、binding proofを使用。統合済みPR #210はunbound / unavailable / ambiguousを分離する。 | CCP2公式外部仕様から個体照合方式は不明。digiCamControlはcamera propertyとsessionを関連付けるが、A0のcross-transport binding相当の証拠ではない。 | A0固有のsoftware実装済み。実機受入は未完了。 |
| 能力確認 | Live View、SaveMedia、JPEG Fine L、SDRAM empty、WPD still-image capability等を事前確認し、未確認はfail-closed。 | digiCamControlは`CaptureInRam`を確認。libgphoto2はadvertised/vendor operationsを補正・確認。 | software実装済み。capability不一致の匿名診断を統合済みPR #210で補強。 |
...
| 後処理 | SaveMedia復元/readback、SDK close、WPD close、process残留確認。card spool時のみ厳格条件後のexact object delete。 | digiCamControlは成功/失敗時にbusyを解除する。D7000の利用者報告には転送失敗後のSDRAM残留例があるが、D810同一現象の証拠ではない（https://github.com/dukus/digiCamControl/issues/155）。 | software実装済み。実機復元証拠は未受入。 |
| 下流表示・合成 | canonical original確定後だけ表示・合成へ渡す。 | CCP2はdisk保存後に表示/他アプリ連携する。 | 設計原則が整合。 |

## A0コードの比較起点

PR #210統合前base `6132f12c...`（PC-direct既存実装の固定参照）:
...
調査時current `main` `c528007f...`はPR #210 head `d767dda8...`を統合済み。以下はPR headの固定参照:
...
- 統合済みPR #210により、WPD検出・capability・identity照合失敗と、後段spool inspection失敗を区別できるsoftware evidenceを追加した。これはsoftware実装済みであり、実機受入済みを意味しない。
...
1. merged `main`から作る固定候補について、実行時点の権限、対象CAM-A、artifact SHA、実行回数、no-retry条件を明示承認・安全gateと再照合した場合だけ、CAM-A単体read-only spool statusを実行し、`enumeratedD810Count`、`stillImageCompatibleCount`、identity category、inventory closeを確認する。
2. 事前条件が全て成立し、その時点で適用可能な明示承認を対象artifact、最大1回、no-retry、SaveMedia復元/readback条件まで再照合した場合だけ、PC-direct transactionで「撮影要求」と「画像通知・転送・原本確定・復元」を別々に証拠化する。
