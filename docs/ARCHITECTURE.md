...
Live Viewは一台選択式であり、二台同時表示は行わない。`hardware.v2`の長寿命agent sessionは明示start、連続frame、heartbeat、stop、20秒idle timeout、600秒max lifetime、request単列化によるbackpressureを持つ。プレビューframeはmemory-onlyの一時表示で、`EvidenceWriter`の原画像、JPEG帰属、合成入力に渡さない。撮影要求が来たらv2 Live Viewを停止しSDK session closeの成功を待つ。次に既存`hardware.v1`でWPD baselineを取得してfull closeし、SDKがカメラカードへ一回だけ撮影してfull closeし、WPDがreopenして撮影commandなしで回収する。v1の有限probeとv2を混同せず、v2はv1 captureへ`liveViewHandoffRequested:false`を送る。停止が失敗・不明ならcapture 0、撮影がverified `Complete`のときだけ新しいv2 sessionを開始する。software contractは実装済みだが、実D810での対話表示と10回handoff受入は未検証である。

M3の`HardwareSingleCamera`画面は、一つのC++ `A0CameraStitcher.CameraAgent.exe`をrandom named pipeで所有し、v1 durable capture/recoveryとv2継続Live Viewを同じprocessへ直列dispatchする。capture前にlocal app stateへ`SingleCamera`、transaction ID、required alias、承認profile ID/version/SHA/expiry、有限v1 handoff intent、dispatch markerをwrite-throughで保存する。応答不明や再起動後は同じIDとsnapshotの`get-transaction-result`だけを実行し、captureを再送しない。`TransactionNotFound`は曖昧状態としてpendingを保持する。canonical originalはアプリ側でも再読込検証し、明示export時は`.partial`へbyte-identical copyし、write/deleteを拒否するhandleで再読込検証して、その同じfile identityをhandle-based renameでpublishする。capture wire・journal・failureは[Hardware Camera Agent v1](HARDWARE_CAMERA_AGENT_V1.md)、継続表示は[Hardware Camera Agent v2](HARDWARE_CAMERA_AGENT_V2.md)に固定する。これらはsoftware contractであり、実D810・actual JPEGの製品受入ではない。
