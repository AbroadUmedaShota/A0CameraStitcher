...
- `FR-UI-001`: 一画面の撮影ダッシュボードで、起動セッションの排他同意、左右カメラの接続・identity・設定・card・Live View状態、リグプロファイルID・版・期限、設置と自動補正可否、処理進捗、撮影・合成・保存を分離した結果、赤Blocker・黄Caution・青Infoを表示できる。
- `FR-UI-002`: `Ready`または`ReadyWithCorrection`の場合だけ追加確認なしの一回操作で撮影し、active transaction中の競合操作と二重開始を禁止できる。設置・校正、read-onlyカメラ設定、別job再合成、結果確認後の明示保存、read-onlyの新規撮影準備を提供し、補正範囲外では測定値付きで物理調整または再校正を案内できる。
