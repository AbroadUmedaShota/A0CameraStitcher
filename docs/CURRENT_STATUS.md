...
- 最新ソースをSDK有効／なしの両構成でクリーンビルドし、各CTest 5/5を確認。M3 simulated Release buildも警告0、契約8/8を再確認。
- M2Pの光学計算、rig-profile trust、三状態setup-assessment contractがsoftware-only合格。
- M3PのNamed Pipe、durable simulated transaction、WPF shellがsimulation-only合格。
...
- M3P: 実Camera Agent、実JPEG、実D810との統合証拠ではない。
