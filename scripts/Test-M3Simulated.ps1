...
try {
    ...
    try {
        ...
        Assert-Condition (($dualCameraFlowOutput -join "`n").Contains('DualCamera flow tests: 16/16 passed.')) 'DualCamera flow test summary is missing or incomplete.'
        ...
        Assert-Condition (($operatorShellTestOutput -join "`n").Contains('Operator shell tests: 22/22 passed.')) 'M3 operator shell test summary is missing or incomplete.'
    }
    finally {
